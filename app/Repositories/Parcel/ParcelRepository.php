...
class ParcelRepository implements ParcelInterface {
    ...
    public function all(){
        ...
        if(!blank($userHubID)){
            return Parcel::with('parcelEvent')->where('hub_id',$userHubID)->orderByDesc('id')->paginate(10);
        }else{
            return Parcel::with('parcelEvent')->orderByDesc('id')->paginate(10);
        }
    ...
    public function filter($request){
        ...
        if(!blank($userHubID)){
           return Parcel::with('parcelEvent')->where('hub_id',$userHubID)->orderByDesc('id')->where(function( $query ) use ( $request ) {
                if($request->parcel_date) {
                    $date = explode('To', $request->parcel_date);
                    if(is_array($date)) {
                        $from   = Carbon::parse(trim($date[0]))->startOfDay()->toDateTimeString();
            ...
            return Parcel::with('parcelEvent')->orderByDesc('id')->where(function( $query ) use ( $request ) {
    ...
    public function store($request) {
        ...
        try {
            ...
            $parcel->save();
            $trakingID                             = Str::upper(settings()->par_track_prefix).substr(strtotime(date('H:i:s')),1).'C'.$parcel->merchant_id.$parcel->id;
    ...
    public function deliverymanStatusParcel($status){
        ...
        if($status == ParcelStatus::DELIVERED):
            return Parcel::orderByDesc('id')->with(['merchant'])->whereIn('status',[ParcelStatus::DELIVERED, ParcelStatus::PARTIAL_DELIVERED])->where(function($query){
                $query->wherehas('parcelEvent',function($eventquery){
                    $eventquery->where('delivery_man_id',Auth::user()->deliveryman->id);
                });
            })->get();
        else:
            return Parcel::orderByDesc('id')->with(['merchant'])->where('status',$status)->where(function($query){
                $query->wherehas('parcelEvent',function($eventquery){
                    $eventquery->where('delivery_man_id',Auth::user()->deliveryman->id);
                });
            })->get();
